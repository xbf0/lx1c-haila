...
#define WIFI_SSID "xiao"    //wifi名称
#define WIFI_PASSWORD "200502233"  //wifi密码
//MQTT三元组
#define ClientId "e590e02eb82b694c002"
#define Username "bkrc"
#define Password "88888888"

#define MQTT_Address "115.28.209.116"
#define MQTT_Port 1883
#define Iot_link_MQTT_Topic_Report "device/873ccaf8f67d4169/up"
#define Iot_link_MQTT_Topic_Commands "device/873ccaf8f67d4169/down"
...
void protocolJSON(unsigned char JsonData[]) {
  ...
  root["sign"] = "873ccaf8f67d4169";
  root["type"] = 3;

  JsonObject& temphumi = data.createNestedObject("Temp_Humi");            //温湿度传感器模块标识
JsonObject& weight = data.createNestedObject("weight");  

  sprintf(printf_buf, "%d", 0);

  temphumi["Temp"] = printf_buf;  //温度模型标识
  temphumi["Humi"] = printf_buf;  //湿度模型标识
  weight["weight_value"] = printf_buf;  //湿度模型标识
  ...
  if (JsonData[0] == 0x55) {
    if (JsonData[1] == 0xAA) {
      ...
      delay(50);
       ...
       sprintf(total_data, "%d", JsonData[7]*256+JsonData[4]);
       sprintf(total_data1, "%d", JsonData[4]);
      weight["weight_value"] = total_data;
      memset(total_data, 0, sizeof(total_data));
      memset(total_data1, 0, sizeof(total_data1));
      delay(50);
